...
def _calculate_cut_index(
    data_shape: Tuple[int, int], tile_size_x: int, tile_size_y: int
) -> Tuple[int, int]:
...
def _separate_image_in_tiles(
    image: np.ndarray, tile_size_x: int, tile_size_y: int
) -> np.ndarray:
    M = tile_size_x
    N = tile_size_y
    image_tiles = [
        image[x:x + M, y:y + N] for x in range(0, image.shape[0], M)
        for y in range(0, image.shape[1], N)
    ]

    image_tiles = np.array(image_tiles)

    return image_tiles.reshape(
        image_tiles.shape[0], image_tiles.shape[1] * image_tiles.shape[2]
    )
...
class TabularizeTimeSeries(BaseEstimator):
    ...
    def transform(
        self, X: InputData, y: Union[pd.DataFrame, pd.Series]
    ) -> Tuple[InputData, TargetData]:
        X = pd.DataFrame(X)
        ...
        cut_index_x, cut_index_y = _calculate_cut_index(
            X.shape, self.tile_size_x, self.tile_size_y
        )
        ...
        labels_tiles = _separate_image_in_tiles(
            labels_reduced, self.tile_size_x, self.tile_size_y
        )
        ...
        # Separate the data and labels in tiles
        training_tiles = _separate_image_in_tiles(
            training_reduced.values, self.tile_size_x, self.tile_size_y
        )
