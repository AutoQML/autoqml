...
from autoqml_lib.optimizer import evaluation
from autoqml_lib.optimizer.metric import Accuracy, BalancedAccuracy, Metric, RMSE
...
from autoqml_lib.search_space.classification import ClassificationChoice
from autoqml_lib.search_space.data_cleaning.imputation import ImputationChoice
from autoqml_lib.search_space.preprocessing.dim_reduction import \
    DimReductionChoice
from autoqml_lib.search_space.preprocessing.downsampling import \
    DownsamplingChoice
from autoqml_lib.search_space.preprocessing.encoding import EncoderChoice
from autoqml_lib.search_space.preprocessing.rescaling import RescalingChoice, RescalingChoiceQML
from autoqml_lib.search_space.regression import RegressionChoice
...
class AutoQML(abc.ABC):
    ...
    def _validate_input_parameters(self, cmd: AutoQMLFitCommand) -> None:
        ...
        config_quantum_regression = {
            'autoqml_lib.search_space.regression.RegressionChoice__choice': copy.copy(all_qc_reg_methods)
        }
        ...
        config_classic_regression = {
            'autoqml_lib.search_space.regression.RegressionChoice__choice': list(RegressionChoice.get_classic_components().keys())
        }

        all_qc_class_methods = list(ClassificationChoice.get_qc_components().keys())
        config_quantum_classification = {
            'autoqml_lib.search_space.classification.ClassificationChoice__choice': copy.copy(all_qc_class_methods)
        }
        ...
        config_classic_classification = {
            'autoqml_lib.search_space.classification.ClassificationChoice__choice': list(ClassificationChoice.get_classic_components().keys())
        }
