...
def generate_gaussian_process_classifier_configurations(
...
def generate_gaussian_process_regressor_configurations(
    cls: type, build_prefix: callable
) -> list[dict]:
...
def generate_decision_tree_classifier_configurations(
    cls: type, build_prefix: callable
) -> list[dict]:
...
def generate_decision_tree_regression_configurations(
    cls: type, build_prefix: callable
) -> list[dict]:
...
def generate_perceptron_classifier_configurations(
    cls: type, build_prefix: callable
) -> list[dict]:
    ...
    for p in penalty:
        configurations.append({build_prefix(cls) + 'penalty': p})

    return configurations


def generate_random_forest_classifier_configurations(
    cls: type, build_prefix: callable
) -> list[dict]:
    criterion = ['gini', 'entropy', 'log_loss']
    ...
    for c in criterion:
        configurations.append({build_prefix(cls) + 'criterion': c})

    return configurations


def generate_random_forest_regression_configurations(
    cls: type, build_prefix: callable
) -> list[dict]:
    criterion = ['squared_error', 'absolute_error', 'friedman_mse']
...
def generate_ridge_classifier_configurations(
    cls: type, build_prefix: callable
) -> list[dict]:
    ...
    for fi in fit_intercept:
        configurations.append({build_prefix(cls) + 'fit_intercept': fi})

    return configurations


def generate_kernel_ridge_configurations(cls: type,
                                         build_prefix: callable) -> list[dict]:
    ...
    for k in kernel:
        configurations.append({build_prefix(cls) + 'kernel': k})

    return configurations


def generate_linear_regression_configurations(
    cls: type, build_prefix: callable
) -> list[dict]:
...
def generate_logistic_regression_configurations(
    cls: type, build_prefix: callable
) -> list[dict]:
...
def generate_svc_configurations(cls: type,
                                build_prefix: callable) -> list[dict]:
...
def generate_svr_configurations(cls: type,
                                build_prefix: callable) -> list[dict]:
...
def generate_quantum_kernel_configurations(
    ...
    for outer_kernel in outer_kernels:
        configurations.append(
            {
                build_prefix(cls) + "quantum_kernel":
                    "projected_quantum_kernel",
                build_prefix(cls) + "measurement":
                    measurement,
                build_prefix(cls) + "outer_kernel":
                    outer_kernel,
                build_prefix(cls) + "parameter_seed":
                    0,
            }
        )
...
def generate_encoding_circuit_configurations(
    ...
    for encoding_circuit in encoding_circuits:
        ...
        if encoding_circuit in [
            "chebyshev_pqc", "chebyshev_rx", "chebyshev_tower"
        ]:
...
def generate_observable_configurations(cls: type,
                                       build_prefix: callable) -> list[dict]:
    observables = [
        "single_pauli_x",
        "single_pauli_y",
...
def generate_full_kernel_configurations(cls: type,
                                        build_prefix: callable) -> list[dict]:
    quantum_kernel_configs = generate_quantum_kernel_configurations(
        cls, build_prefix
    )
    ...
    for encoding_circuit_config in encoding_circuit_configs:
        for ml_model_config in ml_model:
            for operator_config in operators:
                ...
                config.update(encoding_circuit_config)
                config.update(
                    {build_prefix(cls) + "ml_model": ml_model_config}
                )
                config.update(
                    {build_prefix(cls) + "operators": operator_config}
                )
                config.update({build_prefix(cls) + "parameter_seed": 0})
...
@pytest.mark.parametrize(
    "config",
    generate_ridge_classifier_configurations(
        cls=RidgeClassifier, build_prefix=get_fullname
    )
)
...
@pytest.mark.parametrize(
    "config",
    generate_random_forest_classifier_configurations(
        cls=RandomForestClassifier, build_prefix=get_fullname
    )
)
...
@pytest.mark.parametrize(
    "config",
    generate_perceptron_classifier_configurations(
        cls=Perceptron, build_prefix=get_fullname
    )
)
...
@pytest.mark.parametrize(
    "config",
    generate_gaussian_process_classifier_configurations(
        cls=GaussianProcessClassifier, build_prefix=get_fullname
    )
)
...
@pytest.mark.parametrize(
    "config",
    generate_decision_tree_classifier_configurations(
        cls=DecisionTreeClassifier, build_prefix=get_fullname
    )
)
    ...
    config.update(
        {
            ...
            "autoqml.search_space.preprocessing.dim_reduction.DimReductionChoice__choice":
                "no-op",
            "autoqml.search_space.classification.classic.decision_tree_classifier.decision_tree_classifier.DecisionTreeClassifier__max_depth":
                1,
            "autoqml.search_space.classification.classic.decision_tree_classifier.decision_tree_classifier.DecisionTreeClassifier__min_samples_split":
                0.01,
        }
    )
    X_train, y_train = sklearn.datasets.make_classification(
...
@pytest.mark.parametrize(
    "config",
    generate_logistic_regression_configurations(
        cls=LogisticRegressor, build_prefix=get_fullname
    )
)
def test_LogisticRegressor(config: dict):
    ...
    config.update(
        {
            "autoqml.search_space.regression.ClassificationChoice__choice":
...
@pytest.mark.parametrize(
    "config",
    generate_random_forest_regression_configurations(
        cls=RandomForestRegressor, build_prefix=get_fullname
    )
)
...
@pytest.mark.parametrize(
    "config",
    generate_linear_regression_configurations(
        cls=LinearRegressor, build_prefix=get_fullname
    )
)
...
@pytest.mark.parametrize(
    "config",
    generate_kernel_ridge_configurations(
        cls=KernelRidge, build_prefix=get_fullname
    )
)
...
@pytest.mark.parametrize(
    "config",
    generate_decision_tree_regression_configurations(
        cls=DecisionTreeRegressor, build_prefix=get_fullname
    )
)
...
@pytest.mark.parametrize(
    "config",
    generate_gaussian_process_regressor_configurations(
        cls=GaussianProcessRegressor, build_prefix=get_fullname
    )
)
...
@pytest.mark.parametrize(
    "config",
    generate_full_kernel_configurations(cls=QSVR, build_prefix=get_fullname)
)
...
@pytest.mark.parametrize(
    "config",
    generate_full_kernel_configurations(cls=QGPR, build_prefix=get_fullname)
)
...
@pytest.mark.parametrize(
    "config",
    generate_full_kernel_configurations(cls=QKRR, build_prefix=get_fullname)
)
    ...
    config.update(
        {
            ...
            "autoqml.search_space.regression.RegressionChoice__choice":
                "qnnr",
            "autoqml.search_space.regression.quantum.qnnr.QNNR__epochs":
                2,
            "autoqml.search_space.preprocessing.dim_reduction.DimReductionChoice__choice":
                "no-op",
        }
    ...
    config.update(
        {
            ...
            "autoqml.search_space.classification.ClassificationChoice__choice":
                "qnnc",
            "autoqml.search_space.classifictation.quantum.qnnc.QNNC__epochs":
                2,
            "autoqml.search_space.preprocessing.dim_reduction.DimReductionChoice__choice":
                "no-op",
        }
